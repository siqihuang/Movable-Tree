...
Triangle& Triangle::operator *= ( const double d )
{ 
...
double Triangle::operator [] ( int i) const 
{
	assert(!(i < VX || i > VY));
	return n[i];
}
...
double Triangle::SqrLength() const
{ 
	return n[VX]*n[VX] + n[VY]*n[VY]; 
}

Triangle& Triangle::Normalize() // it is up to caller to avoid divide-by-zero
{ 
   double len = Length();
...
{ 
	return Triangle(-a.n[VX],-a.n[VY]); 
}

Triangle operator + (const Triangle& a, const Triangle& b)
{ 
	return Triangle(a.n[VX]+ b.n[VX], a.n[VY] + b.n[VY]); 
}
...
Triangle operator * (const Triangle& a, const double d)
{ 
	return Triangle(d*a.n[VX], d*a.n[VY]); 
}

Triangle operator * (const double d, const Triangle& a)
{ 
	return a*d; 
}

double operator * (const Triangle& a, const Triangle& b)
{ 
...
Triangle operator / (const Triangle& a, const double d)
{ 
	double d_inv = 1.0f/d; return Triangle(a.n[VX]*d_inv, a.n[VY]*d_inv); 
}

vec3 operator ^ (const Triangle& a, const Triangle& b)
{ 
	return vec3(0.0, 0.0, a.n[VX] * b.n[VY] - b.n[VX] * a.n[VY]); 
}

int operator == (const Triangle& a, const Triangle& b)
{ 
	return (a.n[VX] == b.n[VX]) && (a.n[VY] == b.n[VY]); 
}
...
Triangle Prod(const Triangle& a, const Triangle& b)
{ 
	return Triangle(a.n[VX] * b.n[VX], a.n[VY] * b.n[VY]); 
}

double Dot(const Triangle& a, const Triangle& b)
{
	return a*b;
}

...
BV::BV(std::vector<glm::vec3>&all_points)
{
	use_aabb = true;
	if(use_aabb)
	{
...
BV::BV(std::vector<Face*>&all_tris)
{
	...
	//calc min,max_pt
	for (std::vector<Face*>::iterator iter = all_tris.begin(); iter != all_tris.end(); ++iter) 
	{
		for(int i = 0; i < (*iter)->vertex_coords.size(); ++i)
		{
			min_x = std::min(min_x, (*iter)->vertex_coords[i].x);	
			max_x = std::max(max_x, (*iter)->vertex_coords[i].x);	

			min_y = std::min(min_y, (*iter)->vertex_coords[i].y); 
			max_y = std::max(max_y, (*iter)->vertex_coords[i].y); 

			min_z = std::min(min_z, (*iter)->vertex_coords[i].z); 
			max_z = std::max(max_z, (*iter)->vertex_coords[i].z); 
		}
	}
...
void BV::init_box(std::vector<glm::vec3>&all_points)
{
	float min_x, max_x, min_y, max_y, min_z, max_z;
	min_x = min_y = min_z= FLT_MAX;
	max_x = max_y = max_z = -FLT_MAX;
	
	for (std::vector<glm::vec3>::iterator iter = all_points.begin(); iter != all_points.end(); ++iter) 
	{
...
void BV::init_sphere(std::vector<glm::vec3>&all_points)
{
	float min_x, max_x, min_y, max_y, min_z, max_z;
	min_x = min_y = min_z= FLT_MAX;
